...
RUN apt-get update && \
	...
	wget -O VulkanSDK.run https://vulkan.lunarg.com/sdk/download/1.0.49.0/linux/vulkansdk-linux-x86_64-1.0.49.0.run?human=true && \
	chmod ugo+x VulkanSDK.run

RUN	./VulkanSDK.run
RUN	cd VulkanSDK/1.0.49.0

ENV VULKAN_SDK="/VulkanSDK/1.0.49.0/x86_64:${VULKAN_SDK}"
